...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        for i in range(window_queries.shape[1]):
            ...
            energies[:, :, i, start:end] = local_energy
...
class Pose2AudioLongformer(nn.Module):
    ...
    def make_longformer_trg_mask(self, trg, window_size, use_global_token=True):
        ...
        # Use broadcasting to expand the window mask for the batch and heads dimensions
        trg_mask[:, :, (1 if use_global_token else 0):, :] = window_mask[(None if use_global_token else 0):, :]
